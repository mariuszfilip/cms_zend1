...
var $dc=0;
	...
	function sendSMS($from, $to, $message){
		if (strlen($from)>20 || strlen($to)>20) { echo "Length error! "; return false; }
		if(($this->state!="bind_tx") && ($this->state!="bind_tcx")) { echo "State error!"; return false; }
...
$sms['short_message'] = $this->gettext($ar);
	...
	function readPDU(){
		...
		if($this->debug){
			...
			$this->printHex($tmp)." | ".$this->printHex($tmp2)." | ".$this->printHex($body);
			echo "Raw PDU         : $tmp  $tmp2  $body \n";
