...
EOF
    ...
    public function printAccountsList()
    {
        ...
        foreach ($this->accountList as $v) {
            echo "<b>Numer rachunku: " . $this->formatAccountNumber($v['account_details']['account']['number']) . "</b><br>";
            echo "&nbsp; Środki własne: {$v['resources']['balance']['amount']} {$v['resources']['balance']['currency']}<br>";
            echo "&nbsp; Dostępne środki: {$v['resources']['available_resources']['amount']} {$v['resources']['available_resources']['currency']}<br><br>";
